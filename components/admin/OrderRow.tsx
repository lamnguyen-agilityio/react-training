...
// import { useDocumentProjection, type Documentdata } from "@sanity/sdk-react";
...
function OrderRowContent(data: any) {
  if (!data) return null;
  ...
  return (
    <TableRow className="group transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
      ...
      <TableCell className="py-3 sm:py-4">
        <Link href={`/admin/orders/${data.id}`} className="block">
          <div className="flex items-center justify-between gap-2 sm:block">
            <span className="font-medium text-zinc-900 dark:text-zinc-100">
              #{formatOrderNumber(data.id)}
            </span>
            {/* Mobile: Total inline */}
      ...
      <TableCell className="hidden py-4 text-zinc-500 dark:text-zinc-400 sm:table-cell">
        <Link href={`/admin/orders/${data.id}`} className="block truncate">
          {data.userEmail}
        </Link>
      </TableCell>
